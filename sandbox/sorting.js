...
const hikes = [
  ...
  // sort changes the original array, toSorted will not (Don't know how to use that yet)
  const anotherSort = simpleList.sort(compareFn)
  console.log(anotherSort);

  function searchList(list, q){
    function searchCallback(ob){
        //checks to see if a string includes q(query) and returns true or false
        //return string.toLowerCase().includes(q.toLowerCase());
        
        //gets a name out of the object and compares it against the query
        //OR gets the description and checks to see if it has the query
